...
template <typename T>
concept VectorElement = std::assignable_from<T&, T> && std::equality_comparable<T> && std::default_initializable<T>;
#endif
...
{
    if (size () != other.size ()) return false; // diff sizes => different
